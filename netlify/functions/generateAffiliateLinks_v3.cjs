...
async function logToSupabase(level, message, details = {}) {
  try {
    // If supabase client available globally (assigned inside handler), use it
    const sb = globalThis.supabase;
    if (!sb) {
      // fallback to console logging if supabase not available
      console[level === "error" ? "error" : "log"](`[${level}] ${message}`, details || {});
      return;
    }
    // attempt to insert into a lightweight logs table if it exists; ignore failures
    ...
    // do not throw — just console.warn to avoid breaking handler
    try {
...
// -----------------------------
// Main Handler (single entrypoint)
// -----------------------------
exports.handler = async (event, context) => {
  // create supabase client scoped to handler
...
await logToSupabase("info", "Computed travel date ranges", { depart, ret, checkin, checkout });
  ...
  Object.assign(params, buildRentalDates(params.checkin));
    ...
    // ---------------------------------------------------
    try {
      await supabase.from('data_generations').update({ completed_at: new Date(), record_count: linksToInsert.length }).eq('id', generationId);
    } catch (e) {
