...
// --------------------
// Helper: lookup continent + region
// --------------------
async function enrichGeoInfo(country, normalizedCity) {
  let continent = null;
  let region = null;
  let country_code = null;

  if (!country) return { continent, region, country_code };

  try {
    const { data: countryRow, error: countryErr } = await supabase
      .from("country_continent_map")
      .select("continent, country_code")
      .ilike("country_code", country) // fallback if country == code
      .maybeSingle();
    ...
    if (countryRow) {
      ...
      console.error("[enrichGeoInfo] country_continent_map lookup error", countryErr);
    }
  } catch (err) {
    console.error("[enrichGeoInfo] country_continent_map fetch error", err.message);
  }
  ...
  try {
    ...
    if (regionRow) {
      ...
      console.error("[enrichGeoInfo] city_region_map lookup error", regionErr);
    }
  } catch (err) {
    console.error("[enrichGeoInfo] city_region_map fetch error", err.message);
  }

  return { continent, region, country_code };
}
...
export async function handler(event, context) {
  try {
    ...
    console.log("[getFullDestination] Input params", { city, normalizedCity, displayCity, mode, limit, baseUrl });
    ...
    if (cached) {
      console.log("[getFullDestination] Cache hit", { city: normalizedCity });
      return {
        statusCode: 200,
    ...
    console.log("[getFullDestination] Cache miss", { city: normalizedCity });
    ...
    const fetchJson = async (url, label) => {
      ...
      try {
        ...
        if (!resp.ok) {
          let bodyText = null;
          try { bodyText = await resp.text(); } catch (e) { bodyText = `<unreadable body: ${e.message}>`; }
    ...
    const country = firstHotel.country || firstAttraction.country || firstRestaurant.country || null;

    const geoInfo = await enrichGeoInfo(country, normalizedCity);
    ...
    if (insertError) {
      ...
      console.log("[getFullDestination] Cache insert success", { city: normalizedCity });
    }
